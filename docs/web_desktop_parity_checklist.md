...
| GMD-02 cross-game traversal from detail header | Open | Canonicalize neighbors under `GET /games/{id}` via `navigation` fields (`prev_game_id`, `next_game_id`, `current_game_id`, plus jump metadata) and treat this as the sole long-term contract.<br>Mark `GET /games/{id}/neighbors` as **deprecated compatibility route** and remove it after parity cutover once web callers are fully migrated. | Add previous/next game controls and keyboard shortcuts in `web/src/components/games/game-detail.tsx` header area.<br>Ensure router transitions in `web/src/app/(app)/games/[id]/page.tsx` preserve board orientation and active tab. | API: extend `tests/test_game_details_api_regression.py` with first/middle/last game navigation assertions and a temporary deprecation-coverage check for `/games/{id}/neighbors` if still exposed.<br>UI: extend `web/src/components/games/game-detail.test.ts` with cross-game button and shortcut traversal assertions.<br>Contract: extend `web/scripts/check-game-detail-contract.mjs` to enforce `GET /games/{id}.navigation` as canonical and document `/games/{id}/neighbors` deprecation. |

### P1 — Stats drill-down/pivot parity (lines, time usage, rating bands)

| Task ID | Status | Backend endpoint contract changes | Frontend component/page changes | Parity acceptance tests (API + UI) |
| --- | --- | --- | --- | --- |
| STS-01 lines drill-down context parity | In progress | Canonicalize on existing `GET /lines/stats/{line_id}` for line detail (`{ line_id, total_games, compliance_rate, ... }`) and `GET /lines/stats/{line_id}/history` for trend buckets (`{ line_id, buckets, totals }`).<br>Do not introduce `/lines/{line_id}` aliases; if present, treat them as non-canonical compatibility paths. | Add row click-through from `/lines` table in `web/src/app/(app)/lines/page.tsx` to a detail drawer/panel rendered by `web/src/components/stats/stats-table.tsx` integration hooks. | API: keep `tests/test_lines_stats_api.py` as the parity contract test for detail + history route/payload shape.<br>UI: add `web/src/components/stats/stats-table.test.ts` for row selection, detail rendering, and history tab toggles.<br>Contract: keep `web/scripts/check-lines-contract.mjs` aligned to `/lines/stats/{line_id}` and `/lines/stats/{line_id}/history`. |
