...
export default function GameDetailPage({
  ...
  params: { id: string };
  searchParams: { ply?: string; tab?: string; orientation?: string };
}) {
  const gameId = Number(params.id);
  const initialPly = searchParams.ply ? Number(searchParams.ply) : null;
  const initialTab = searchParams.tab === "moves" ? "moves" : "board";
  const initialOrientation = searchParams.orientation === "black" ? "black" : "white";
  if (!Number.isFinite(gameId)) return <p className="text-sm text-danger">Invalid game id.</p>;
  return <PageContainer title={`Game ${gameId}`} description="Inspect move quality, evaluation changes, and sideline opportunities for a single game."><PageSection><SectionHeader title={`Game ${gameId}`} description="Inspect move quality, evaluation changes, and sideline opportunities." /><GameDetail gameId={gameId} initialPly={Number.isFinite(initialPly) ? initialPly : null} initialTab={initialTab} initialOrientation={initialOrientation} /></PageSection></PageContainer>;
}
