  ...
  return (
    <PageContainer title="Rating Bands" description="Compare performance by rating segments and adjust the resolution of the report.">
      <PageSection>
        ...
        <label className="grid max-w-xs gap-2 rounded-2xl border border-border bg-panel px-4 py-4 text-sm text-text-subtle shadow-soft">Band size<Select value={normalizedBandSize} onChange={(e) => setBandSize(Number(e.target.value))}>{ALLOWED_BAND_SIZES.map((size) => <option key={size} value={size}>{size}</option>)}</Select></label>
        <StatsTable title="Rating bands" description="Aggregated stats by white/black rating bands." queryKey={["rating-band-stats", String(normalizedBandSize)]} queryFn={() => listRatingBandStats(normalizedBandSize)} />
      </PageSection>
    </PageContainer>
  );
}
