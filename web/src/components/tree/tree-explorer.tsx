...
export function TreeExplorer() {
  ...
  return <Card><label className="grid max-w-xs gap-2 text-sm text-text-subtle">Position id<Input type="number" value={posId} onChange={(e) => setPosId(Number(e.target.value) || 1)} /></label><p className="text-sm text-text-subtle">Coverage {canonical.coveragePct.toFixed(1)}% · rep {canonical.repertoireCount} · games {canonical.gameCount} · top {metrics.data?.top_game_branches.length ?? 0}</p></Card>;
}
